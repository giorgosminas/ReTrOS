function [profile, samples, posteriors, params] = ReTrOSswitch_mRNA(geneLocus,rawData,timescale,replicates,iterations,varargin)
...
% 'figHandle', <graphic handle scalar> - handle to the figure to use for plotting
...
% 'mRNADegradationRatePrior', <real scalar> - sets the alpha and beta parameters of the Gamma prior distribution for the degradation rate (default: [1.4828 0.1823])
...
prior_deg = [1.4828 0.1823]; %Plant Cell. 2007 Nov;19(11):3418-36
...
while currArg <= nvarargin
    ...
    elseif strcmpi(varargin{currArg},'mRNADegradationRatePrior')
        %we expect a single following argument, either a string, or a 2-element non-negative float vector
        if (currArg + 1) > nvarargin
    ...
    elseif strcmpi(varargin{currArg},'maxSamplesToPlot')
        %we expect a single argument, a non-negative integer, specifying maximum number of samples to use for plotting (warning do not plot too many!)
        if (currArg + 1) > nvarargin
...
if sum(strcmpi(regressionMethod,regressionTypes)) == 0
    disp(['Unknown regression method "' regressionMethod '"']);
    dips('Using "leastsquares" as regression method');
end
...
minswTimes = switchDelay;
maxDegRate = 3;    %min half-life: log(2) / 3  < 10 minutes
minDegRate = 0.01;  %max half-life: log(2) / 0.025 > 24 hours
...
maxInitialExp = maxData; %should estimate these from data!
minInitialExp = minData;  %should estimate these from data!


degRate = fastnormrnd(0.75,0.25,1);   %degradation rate
degRate = min(degRate,maxDegRate);
degRate = max(degRate,minDegRate);
...
degRateHistory = zeros(iterations,1);
%initialExpHistory = zeros(iterations,1);
swTimesHistory = cell(iterations,1);
swTimesHistory{iterations} = [];
%birthRatesHistory = cell(iterations,1);
%birthRatesHistory{iterations} = [];
sigma2History = zeros(iterations,1);
%fitHistory = zeros(iterations,timepoints);
...
% if ~explorePrec
%     disp('need to sort out prec!');
%     if replicates > 1
%         prec = 1/mean(std(data).^2);
%     else
prec = 1;
%     end
% end
...
while n <= iterations && ~earlyConverged
    ...
    [initialExp, birthRates] = calculateModel_mRNA(timescale,swTimes,degRate,Y,timepoints,replicates,useDatapoint,Xmatrix,regression);
    profile = nStateSwitchODE_mRNA(timescale,initialExp,birthRates,degRate,swTimes);
    
    %calculate SSE of profile to data
    e_hat = calculateEHat(profile,data,eHatMat,replicates,useDatapoint);
    ...
    lLikelihood = logLikelihood(e_hat,prec,regression);
    
    %update delta - MH
    %degRateProposed = exp( log(degRate) + (rand(1)/2 - 0.25) );
    if useLogExplore
        degRateProposed = exp(log(degRate) + fastnormrnd(0,0.25,1));
        %         while degRateProposed < minDegRate || maxDegRate < degRateProposed
        %             degRateProposed = exp(log(degRate) + fastnormrnd(0,0.25,1));
        %         end
        proposalRatio = degRateProposed / degRate;
    else
        degRateProposed = degRate + fastnormrnd(0,0.25,1);
        while degRateProposed < minDegRate || maxDegRate < degRateProposed
            degRateProposed = degRate + fastnormrnd(0,0.25,1);
        end
        proposalRatio = 1;
    ...
    [initialExpProposed, birthRatesProposed] = calculateModel_mRNA(timescale,swTimes,degRateProposed,Y,timepoints,replicates,useDatapoint,Xmatrix,regression);
    profileProposed = nStateSwitchODE_mRNA(timescale,initialExpProposed,birthRatesProposed,degRateProposed,swTimes);
    e_hat = calculateEHat(profileProposed,data,eHatMat,replicates,useDatapoint);
    lLikelihoodProposed = logLikelihood(e_hat,prec,regression);
    
    lLikelihoodRatio = exp(lLikelihoodProposed - lLikelihood);
    priorRatio = gampdf(degRateProposed,prior_deg(1),prior_deg(2)) /  gampdf(degRate,prior_deg(1),prior_deg(2));
    
    %acceptance: likelihood ratio * prior proposal * proposal
    acceptanceDegRate = min(1, lLikelihoodRatio * priorRatio * proposalRatio);
    
    if sum(birthRatesProposed < 0) > 0
        noFail = 0;
    else
        noFail = 1;
    end
    acceptanceDegRate = acceptanceDegRate * (initialExpProposed >= minInitialExp) * (initialExpProposed <= maxInitialExp) * noFail;
    %     else
    %         acceptanceDegRate = 0;
    %     end
    
    if rand(1) <= acceptanceDegRate
        degRate = degRateProposed;
    ...
    if r <= pMove
        ...
        if range > 0
            ...
            [initialExpProposed, birthRatesProposed] = calculateModel_mRNA(timescale,swTimesProposed,degRate,Y,timepoints,replicates,useDatapoint,Xmatrix,regression);
            profileProposed = nStateSwitchODE_mRNA(timescale,initialExpProposed,birthRatesProposed,degRate,swTimesProposed);
            e_hat = calculateEHat(profileProposed,data,eHatMat,replicates,useDatapoint);
            lLikelihoodProposed = logLikelihood(e_hat,prec,regression);
    ...
    if acceptanceSwitch == 0 || sum(birthRatesProposed < 0) > 0
        noFail = 0;
    else
        noFail = 1;
    end
    ...
    if acceptanceSwitch > 0
        acceptanceSwitch = acceptanceSwitch * (initialExpProposed >= minInitialExp) * (initialExpProposed <= maxInitialExp) * noFail;
    end
    ...
    [initialExp, birthRates] = calculateModel_mRNA(timescale,swTimes,degRate,Y,timepoints,replicates,useDatapoint,Xmatrix,regression);
    %fitHistory(n,:) = nStateSwitchODE_mRNA(timescale,initialExp,birthRates,degRate,swTimes);
    lLikelihoodHistory(n) = lLikelihood;
    %initialExpHistory(n) = initialExp;
    swTimesHistory{n} = swTimes + timescaleShift;
    %birthRatesHistory{n} = birthRates;
    
    sigma2History(n) = 1 / prec;
    
    degRateHistory(n) = degRate;
    ...
    %29.07.2014
    %THIS IS LIKELY TO BE COMPLETELY DEPRECATED SHORTLY
...
params.mRNADegradationRatePrior = prior_deg;
...
%construct sample history structure
samples = struct;
samples.degradationRate_mRNAHistory = degRateHistory;
samples.swTimesHistory = swTimesHistory;
%samples.tausHistory = birthRatesHistory;
%samples.M_0History = initialExpHistory;
samples.logLikelihoodHistory = lLikelihoodHistory;
samples.sigma2History = sigma2History;
%samples.fit_mRNAHistory = fitHistory;
samples.lLikelihoodHistory = lLikelihoodHistory;
%samples.regression = regression;
if strcmpi(regression.type,'weightedleastsquares')
...
if isPlot
    ...
    if plotDegDist
        figure('Name','Degradation rate distributions','NumberTitle','off','WindowStyle','docked');
        t = 0.001:0.01:3;
        
        hold on;
        plot(t,gampdf(t,prior_deg(1), prior_deg(2)),'--k','linewidth',2);
        %if fit.posteriors.switchDistribution.switchNumber>0
        m = posteriors.degradationRate.mean;
        v = posteriors.degradationRate.standardDeviation^2;
...
% function [initialExp birthRates] = calculateModelMissingData(timescale,swTimes,degRate,Y,timepoints,replicates,useSamples,Xmatrix)
